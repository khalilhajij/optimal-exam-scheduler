...
courses_per_semester = {
    ...
    "5th Semester": ["CS340", "CS331", "SE322", "CYS401", "ISC203", "ARAB103", "PE"],
...
for semester, courses in courses_per_semester.items():
    for i in range(len(courses)):
        for j in range(i + 1, len(courses)):
            ...
            # Ensure there's at least a one-day gap between exams in the same semester
            model.Add(exam_day_1 != exam_day_2)
...
# Ensure the solution fully utilizes the 12-day period
for course, var in exam_slot_vars.items():
    model.Add(var < len(exam_dates) * len(time_slots))
